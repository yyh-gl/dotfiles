...
`claude/managed-settings.json`は`nix/darwin/claude-code.nix`のactivation scriptで`/Library/Application Support/ClaudeCode/managed-settings.json`（root所有、Claude Codeからは書き込み不可）に配置している。Claude Codeは`/model`・`/effort`・plugin installなどの操作で`~/.claude/settings.json`を手動追加フィールドごと丸ごと再生成してしまう既知バグ（[claude-code#22659](https://github.com/anthropics/claude-code/issues/22659)）があり、home-manager経由のsymlink配置では設定が消えてしまうため、絶対に保持したい設定（permissions・hooks・sandbox・statusLineなど）はこちらに置く。`enabledPlugins`・`effortLevel`のようなClaude Code自身が書き換える可変設定は`~/.claude/settings.json`側に残し、Nixでは管理しない（`extraKnownMarketplaces`は公式marketplaceの登録を固定するためmanaged-settings.json側に置いている）。`advisorModel`も同様に`/advisor`コマンドで上書きされてしまうため、「advisorとして常にFableを使う」というポリシーを固定する目的でこちらに置いている（[advisor tool](https://code.claude.com/docs/en/advisor)はメインモデルが行き詰まったときなどにのみ内部で呼び出す仕組みのため、常時Fableを使う既存のagent達とは異なり「適切なときにのみ」Fableを使う設計になる）。Claude Code側にadvisor専用のフォールバックチェーン機能はないため、Fable未対応環境（Claude Code v2.1.170未満、または組織にFable 5アクセスがない）ではadvisorが単に無効化される（エラーにはならない）。その場合は`fallbackModel`（メインモデル用）がOpusへのフォールバックを担う。
