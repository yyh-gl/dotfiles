...
- 数行程度の変更で調査もほぼ不要な場合、チーム起動のやり取りコストが実装コスト自体を
  上回ることがある。その場合はチームを起動せず、メインセッションでの直接実装で良いかを
  ユーザーに確認する
...
2. Plannerのプランをもとに、Implementerにテストリスト作成 + TDDサイクルを割当。
   Implementerへの割当メッセージには必ずプランのファイルパスを含める（Plannerが調べた
   既存パターン・アーキテクチャ制約をImplementerが自力で再調査しない。同じ調査を
   二重に行うとトークンの無駄になる）
...
| Agent | 役割 | 書き込み権限 | モデル |
|-------|------|------------|--------|
| Lead | オーケストレーター | なし（読み取り専用） | fable |
| Planner | 実装計画作成（Largeタスク向け） | なし（読み取り専用） | fable |
