...
ClaudeとCodexが独立してレビューを行い、両者の視点を統合した最終レビューを提示するスキル。
...
ファイルパスが指定された場合は `Read` ツールで読み込む。インラインで貼られた場合はそのまま使用する。 大規模なコードベースの場合は、関連する主要ファイルに絞って対象を明示する。

### Step 2: Claudeによる独立レビュー
...
### Step 3: Codexによる独立レビュー

`mcp__codex__codex`を使い、Codexに同じ観点でレビューを依頼する。
...
1. 総評（2〜3文）
2. 指摘事項（Critical / Major / Minor / Positiveに分類）
3. 推奨事項（具体的なアクション）
```

Codexの回答を受け取り、追加の確認が必要な場合は`mcp__codex__codex-reply`で続ける。

### Step 4: 両者の結果を比較・統合

ClaudeとCodexの指摘を突き合わせる：

| 分類           | 意味          | 扱い             |
|--------------|-------------|----------------|
| **両者一致**     | 両方が同じ問題を指摘  | 最優先・高信頼度       |
| **Claudeのみ** | Claudeだけが指摘 | 記載（出典: Claude） |
| **Codexのみ**  | Codexだけが指摘  | 記載（出典: Codex）  |
...
[両者の評価を統合した3〜4文のサマリー。全体的な品質と最重要ポイントを伝える]
...
- **Claude**: [Claudeの全体的な評価・着目点を1〜2文で]
- **Codex**: [Codexの全体的な評価・着目点を1〜2文で]

---

## 注意事項

- Codexへの送信は外部サービスへのデータ送信である。送信前にレビュー対象へシークレット（APIキー・トークン・秘密鍵）・認証情報・個人情報が含まれていないか確認し、含まれる場合は該当箇所をマスクするか、ユーザーに送信可否を確認する。
- Codexが応答を返さない・エラーになる場合は、Claude単独のレビュー結果を提示し、その旨を明記する。
